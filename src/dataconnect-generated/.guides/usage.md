...
```ts
import { useCreateNewPlaylist, useGetPublicPlaylists, useAddSongToPlaylist, useAddLikeToSong } from '@dataconnect/generated/react';
// The types of these hooks are available in react/index.d.ts

const { data, isPending, isSuccess, isError, error } = useCreateNewPlaylist(createNewPlaylistVars);

const { data, isPending, isSuccess, isError, error } = useGetPublicPlaylists();

const { data, isPending, isSuccess, isError, error } = useAddSongToPlaylist(addSongToPlaylistVars);

const { data, isPending, isSuccess, isError, error } = useAddLikeToSong(addLikeToSongVars);
